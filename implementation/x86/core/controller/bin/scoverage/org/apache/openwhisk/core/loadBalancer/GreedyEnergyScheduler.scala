...
object GreedyEnergyScheduler extends CommonScheduler {
  ...
  override def scheduleFunction(
    ...
    if (numInvokers > 0) {
      ...
      val sortedInvokers = invokers.sortBy {invoker => if(((invoker.energyProfile.currBat + invoker.energyProfile.panelOutput)/totalEnergy).isNaN) 0.0 else -(invoker.energyProfile.currBat + invoker.energyProfile.panelOutput)/totalEnergy}
