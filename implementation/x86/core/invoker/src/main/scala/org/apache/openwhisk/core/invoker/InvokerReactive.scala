...
class InvokerReactive(
  ...
  Scheduler.scheduleWaitAtMost(1.seconds)(() => {
    // val osBean: OperatingSystemMXBean = (OperatingSystemMXBean) ManagementFactory.getOperatingSystemMXBean()
    ...
    val cpuLoadPercentage: Double = osBean.getSystemCpuLoad()
    val cpuLoadPercentage2: Double = osBean.getProcessCpuLoad()
    ...
    currentEnergy.andThen {
      case Success(value) => {
        ...
        logging.info(this, s"Pinged the controller with panelOutput: ${value.panelOutput}, " +
          s"currBat: ${value.currBat}, batRatio: ${value.batRatio}, excessEnergy: ${value.excessEnergy}, cpuUtil: ${cpuLoadPercentage}, cpuUtil2: ${cpuLoadPercentage2}")
