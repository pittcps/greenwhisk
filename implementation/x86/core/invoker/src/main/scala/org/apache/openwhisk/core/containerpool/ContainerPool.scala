...
class ContainerPool(childFactory: ActorRefFactory => ActorRef,
  ...
  var prewarmStartCount = immutable.Map.empty[StartKey, Int]
              ...
              .orElse(
                ...
                if (hasPoolSpaceFor(busyPool ++ freePool, r.action.limits.memory.megabytes.MB)) {
                  takePrewarmContainer(r.action)
                    .map(container => {
                      incrementStartCount(r.action.exec.kind, r.action.limits.memory.megabytes.MB, "prewarmed")
                      (container, "prewarmed")
                    })
                    .orElse {
  ...
  def adjustPrewarmedContainer(init: Boolean, scheduled: Boolean): Unit = {
    ...
    if (scheduled) {
      logging.info(this, s"Total number of prewarmed starts in this minute: ${prewarmStartCount.values.sum}")
      ...
      prewarmStartCount = immutable.Map.empty[StartKey, Int]
    ...
    prewarmConfig
      ...
      .foreach { _ =>
        ...
        key match {
          ...
          case "prewarmed" =>
            prewarmStartCount.get(startKey) match {
              case Some(value) => prewarmStartCount = prewarmStartCount + (startKey -> (value + 1))
              case None => prewarmStartCount = prewarmStartCount + (startKey -> 1)
            }
          case "warm" =>
