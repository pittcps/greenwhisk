...
class InvokerEnergyProfile(
  ...
  def updateEnergyProfile(cpuUtil: Double): EnergyProfile = {
    val powerNeededByInvoker = EnergyProfileConstants.IDLE_POWER  +
      cpuUtil * (EnergyProfileConstants.MAX_POWER - EnergyProfileConstants.IDLE_POWER)
  ...
  def getCurrentEnergyProfile(): EnergyProfile = {
    ...
    val battRatio = Try(currBatAvail / batterySize).getOrElse(0.0)
    val result = if (battRatio.isNaN) 0.0 else battRatio

    EnergyProfile(getPowerFromPanel(), currBatAvail, result, excessEnergy)
  }
