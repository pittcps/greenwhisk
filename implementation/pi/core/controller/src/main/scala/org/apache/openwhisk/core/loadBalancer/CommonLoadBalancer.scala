...
abstract class CommonLoadBalancer(config: WhiskConfig,
  ...
  protected def processResult(aid: ActivationId,
    ...
    response match {
      case Left(activationId: ActivationId) => logging.info(this, s"Repsonse is ${activationId.toString}")
      case Right(whiskActivation: WhiskActivation) => logging.info(this, s"Repsonse is ${whiskActivation.response.result}")
    }
