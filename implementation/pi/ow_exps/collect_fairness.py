...
translate = {
    'consistent_hashing_logs': 'consistent hashing',
    'baseline_logs': 'baseline',
    'greedy_logs': 'greedy',
    'weighted_dist_logs': 'weighted dist'
}
...
if __name__ == '__main__':
    ...
    if not os.path.isdir('./fairness_graphs'):
        os.mkdir('./fairness_graphs/')

    # Plot avg energy plots
    for key in avg_energy_map.keys():
        # Create the plot
        plt.plot(range(len(avg_energy_map[key])), avg_energy_map[key], label=translate[key])
    ...
    plt.savefig('./fairness_graphs/avg_energy_graph.png')
    plt.close()

    # Plot gini index energy plots
    for key in gini_energy_map.keys():
        # Create the plot
        plt.plot(range(len(gini_energy_map[key])), gini_energy_map[key], label=translate[key])
    ...
    # Save the plot to a file
    plt.savefig('./fairness_graphs/gini_index_energy_graph.png')
    plt.close()

    # Plot coeff of variation energy plots
    for key in coeff_variation_energy_map.keys():
        # Create the plot
        plt.plot(range(len(coeff_variation_energy_map[key])), coeff_variation_energy_map[key], label=translate[key])
    ...
    # Save the plot to a file
    plt.savefig('./fairness_graphs/cv_energy_graph.png')
    plt.close()

    # Plot gini index of function distribution plots
    for key in gini_dist_map.keys():
        # Create the plot
        plt.plot(range(len(gini_dist_map[key])), gini_dist_map[key], label=translate[key])
    ...
    # Save the plot to a file
    plt.savefig('./fairness_graphs/gini_index_dist_graph.png')
    plt.close()

    # Plot gini index energy plots
    for key in coeff_variation_dist_map.keys():
        # Create the plot
        plt.plot(range(len(coeff_variation_dist_map[key])), coeff_variation_dist_map[key], label=translate[key])
    ...
    # Save the plot to a file
    plt.savefig('./fairness_graphs/cv_dist_graph.png')
    plt.close()
